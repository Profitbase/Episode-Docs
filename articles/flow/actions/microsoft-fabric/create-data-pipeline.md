...
| Definition    | Optional | If you want to create a non-empty Data Pipeline that contains actions, properties, parameters, etc, use the Definition property to specify the JSON configuration of the pipeline. You can also specify the JSON configuration as a Base64 encoded string. See [Example 1 below](#example-1---definition). You can get the pipeline definitions via the **View --> View JSON code** tab in the Fabric user interface. |
